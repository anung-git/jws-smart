...
#define PIN_DMD_nOE       15    //PB7 D9 active low Output Enable, setting this low lights all the LEDs in the selected rows. Can pwm it at very high frequency for brightness control.
#define PIN_DMD_A         28    //PC0 D6
#define PIN_DMD_B         29    // D7
#define PIN_DMD_C         30    // D6
#define PIN_DMD_D         31    // D7
#define PIN_DMD_CLK       8     // D13_SCK  is SPI Clock if SPI is used
#define PIN_DMD_SCLK      34    // pc6
#define PIN_DMD_R_DATA    42    // D11_MOSI is SPI Master Out if SPI is used
...
#define LIGHT_DMD_ROW_00()       { PORTC &= ~_BV(PC3); PORTC &= ~_BV(PC2); PORTC &= ~_BV(PC1);  PORTC &= ~_BV(PC0);; }
#define LIGHT_DMD_ROW_01()       { PORTC &= ~_BV(PC3); PORTC &= ~_BV(PC2); PORTC &= ~_BV(PC1); PORTC |= _BV(PC0);; }
#define LIGHT_DMD_ROW_02()       { PORTC &= ~_BV(PC3); PORTC &= ~_BV(PC2); PORTC |= _BV(PC1);;  PORTC &= ~_BV(PC0);; }
#define LIGHT_DMD_ROW_03()       { PORTC &= ~_BV(PC3); PORTC &= ~_BV(PC2); PORTC |= _BV(PC1);; PORTC |= _BV(PC0);; }
#define LIGHT_DMD_ROW_04()       { PORTC &= ~_BV(PC3); PORTC |= _BV(PC2);; PORTC &= ~_BV(PC1);  PORTC &= ~_BV(PC0);; }
#define LIGHT_DMD_ROW_05()       { PORTC &= ~_BV(PC3); PORTC |= _BV(PC2);; PORTC &= ~_BV(PC1); PORTC |= _BV(PC0);; }
#define LIGHT_DMD_ROW_06()       { PORTC &= ~_BV(PC3); PORTC |= _BV(PC2);; PORTC |= _BV(PC1);;  PORTC &= ~_BV(PC0);; }
#define LIGHT_DMD_ROW_07()       { PORTC &= ~_BV(PC3); PORTC |= _BV(PC2);; PORTC |= _BV(PC1);; PORTC |= _BV(PC0);; }
#define LIGHT_DMD_ROW_08()       { PORTC |= _BV(PC3);; PORTC &= ~_BV(PC2); PORTC &= ~_BV(PC1);  PORTC &= ~_BV(PC0);; }
#define LIGHT_DMD_ROW_09()       { PORTC |= _BV(PC3);; PORTC &= ~_BV(PC2); PORTC &= ~_BV(PC1); PORTC |= _BV(PC0);; }
#define LIGHT_DMD_ROW_10()       { PORTC |= _BV(PC3);; PORTC &= ~_BV(PC2); PORTC |= _BV(PC1);;  PORTC &= ~_BV(PC0);; }
#define LIGHT_DMD_ROW_11()       { PORTC |= _BV(PC3);; PORTC &= ~_BV(PC2); PORTC |= _BV(PC1);; PORTC |= _BV(PC0);; }
#define LIGHT_DMD_ROW_12()       { PORTC |= _BV(PC3);; PORTC |= _BV(PC2);; PORTC &= ~_BV(PC1);  PORTC &= ~_BV(PC0);; }
#define LIGHT_DMD_ROW_13()       { PORTC |= _BV(PC3);; PORTC |= _BV(PC2);; PORTC &= ~_BV(PC1); PORTC |= _BV(PC0);; }
#define LIGHT_DMD_ROW_14()       { PORTC |= _BV(PC3);; PORTC |= _BV(PC2);; PORTC |= _BV(PC1);;  PORTC &= ~_BV(PC0);; }
#define LIGHT_DMD_ROW_15()       { PORTC |= _BV(PC3);; PORTC |= _BV(PC2);; PORTC |= _BV(PC1);; PORTC |= _BV(PC0);; }

#define LATCH_DMD_SHIFT_REG_TO_OUTPUT() {PORTC |= _BV(PC6); PORTC &= ~_BV(PC6);}// digitalWrite( PIN_DMD_SCLK, HIGH ); digitalWrite( PIN_DMD_SCLK,  LOW ); }
#define OE_DMD_ROWS_OFF()         {PORTB |= _BV(PB7); }// digitalWrite( PIN_DMD_nOE, HIGH  ); }
#define OE_DMD_ROWS_ON()          { PORTB &= ~_BV(PB7); }//digitalWrite( PIN_DMD_nOE, LOW ); }
...
class DMD
{
  ...
  void drawMarquee( const char* bChars, int length, int left, int top);
