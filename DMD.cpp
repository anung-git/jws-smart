...
void DMD::writePixel(unsigned int bX, unsigned int bY, byte bGraphicsMode, byte bPixel){
    ...
    case GRAPHICS_NORMAL:
	    if (bPixel == true)	bDMDScreenRAM[uiDMDRAMPointer] &= ~lookup;	// zero bit is pixel on
	    else bDMDScreenRAM[uiDMDRAMPointer] |= lookup;	// one bit is pixel off
	    break;
    case GRAPHICS_INVERSE:
	    if (bPixel == false)  bDMDScreenRAM[uiDMDRAMPointer] &= ~lookup;	// zero bit is pixel on
	    else bDMDScreenRAM[uiDMDRAMPointer] |= lookup;	// one bit is pixel off
	    break;
...
void DMD::drawString(int bX, int bY, const char *bChars, int length,byte bGraphicsMode){
...
void DMD::drawMarquee(const char *bChars, int length, int left, int top){
    marqueeWidth = 0;
    for (int i = 0; i < length; i++) {
...
boolean DMD::stepMarquee(int amountX, int amountY){
    ...
    if (amountY==0 && amountX==-1) { // Shift entire screen one bit
...
void DMD::scanDisplayBySPI(){
   ...
   //if(  HIGH ){
        ...
        for (int i=0;i<rowsize;i++) {
            //    SPI.transfer(bDMDScreenRAM[offset+i+row3]);//48
            //    SPI.transfer(bDMDScreenRAM[offset+i+row2]);//32
            //    SPI.transfer(bDMDScreenRAM[offset+i+row1]);//16
            shiftOutFast(bDMDScreenRAM[offset+i]);// bDMDScreenRAM[offset+i]);
            // SPI.transfer(255-(bDMDScreenRAM[offset+i]));//0
        }
        OE_DMD_ROWS_OFF();
...
for(byte b=0;b<8;b++){
  if((_val&0x80)==0x80) PORTA &= ~_BV(PA2); 
  else  PORTA |= _BV(PA2); 
  _val=(_val<<1)|(_val>>7);
  PORTB |= _BV(PB0);  //1
  PORTB &= ~_BV(PB0);  //0
